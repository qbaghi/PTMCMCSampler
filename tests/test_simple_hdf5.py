...
from gibbs import PTMCMCSampler
...
class TestSimpleSampler(TestCase):
    ...
    def test_simple(self):
        ...
        sampler = PTMCMCSampler.PTSampler(
            ndim, glo.lnlikefn, glo.lnpriorfn, np.copy(cov), outDir="./chains",
            hdf5=hdf5)
        ...
        sampler.sample(
            p0, 10000, burn=500, thin=1, covUpdate=500, SCAMweight=20, AMweight=20, DEweight=20,
            isave=1000)
        
        # if pickling:
        #     with open("./chains/chain_1.p", "rb") as fid:
        #         chaindata = pickle.load(fid)
        # with h5py.File("./chains/chain_1.h5", 'r') as hf:
        #     chaindata = hf["chains"][()]
        # else:
        #     chaindata = np.loadtxt("./chains/chain_1.txt")

        # print(chaindata.shape)